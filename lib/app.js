...
function Book(title, author, totalPages, hasRead) {
  ...
  this.currentPage = this.hasRead == false ? 1 : this.totalPages;
  myLibrary.push(this);
}
...
Book.prototype.createBookFrame = function() {
  ...
  this.pagesReadLabel.textContent = `Pages: 1 / ${this.totalPages}`;
...
Book.prototype.incrementDecrementPagesRead = function(e) {
  ...
  if (myLibrary[bookIndex].currentPage > 0 && myLibrary[bookIndex].currentPage < totalPages) {
    pagesReadLabel.textContent = actionType == "increment" ? `Pages: ${myLibrary[bookIndex].currentPage += 1} / ${totalPages}`
    : `Pages: ${(myLibrary[bookIndex].currentPage - 1) == 0 ? False : myLibrary[bookIndex].currentPage -= 1} / ${totalPages}`;
  }
}
